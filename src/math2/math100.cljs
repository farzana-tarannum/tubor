...
(defn alkine [n1]
  [:g {:transform (m7/tranfrom [[:scale [0.7 0.7]]
                                         [:rotate 10]])}
   (let [n n1]
     [:g
  ...
  [:marker {:id a
   ...
   [:path {:d (m7/path [0 0 :L 5 -5
           ...
           :style {:fill-rule :evenodd
                   ...
                   :fill-opacity 1}
           :transform (m7/tranfrom [[:scale 0.4]
                                    [:rotate 180]
                                    [:translate [12.5 0]]])
           }]]
  )
...
(defn template3 []
  [:div {:style
    ...
    #_[:circle {:cx 85 :cy -80 :r 15 :fill (hsl [0 70 70 0.7])}
      [:animateTransform
       {:id :sq
        :attributeName :transform
        :begin :click
     ...
     [:path {:d (path [-90 0 :l 20 0 0 -5 -5 0 0 -5 -10 0 0 5 -5 0 0 5])
             ...
             :stroke (hsl [1 70 70 0.8])
             :stroke-width 1.1
             :fill (hsl [1 70 70 0.9])}
      [:animateTransform
       {:id :sq
        :attributeName :transform
        :begin :click
...
(defn home-work []
    ...
    [:div {:style (merge
     ...
     [:div {:style (m7/css
      ...
      #_[:div
       ...
       ". He fires the arrow (mass = 0.2 kg) vertically upward.
 ...
 the gravitational PE at its highest point"]
      ...
      #_[m7/m '[= PE mgh]]
;;       [m7/m '[= PE [* 0.2 kg 9.8 [m [:p s 2]] h ]  Elastic-enrgy ]]

;;       [m7/m '[=  [* 0.2 kg 9.8 [m [:p s 2]] h ] [:m 50 joules]]]
      ...
      [:svg {:style {:height "100%"
       ...
       [:marker {:id (name :dot222)
        ...
        [:path {:d (m7/path [-3.7
                ...
                :stroke (hsl [5 70 70 1])
                :stroke-width 0.2
                :transform (m7/tranfrom [[:scale [0.6 0.6]]
                                         [:rotate 0]])
              ...
              [:animateTransform {:id :arrow1
                                  ...
                                  :values (m7/sami-colon
                                           (map
                                            m7/space
                                            [[-50 0] [100 0] [160 0]]))
                                  :keyTimes (m7/sami-colon [0.0 0.4 1])
                                  ;;:from (m7/space [0 0])
                                  ;;:to (m7/space [200 0])
                                  :fill :freeze}])
...
.                 x))
      ...
      #_[m7/m ['= 'a   [:m  (/ (- 7.76 4.43) 0.29) ['m [:p 's 2]]] ]]


      #_[m7/m ['= [:m 'sin tt] ['a 'c]] ]
      #_[m7/m ['= [:m 'sin 35] [70 'c] 0.573]]
      #_[m7/m ['= [70 0.573] 'c]]

      #_[m7/m ['= (js/Math.round (/ 70 0.573)) 'c]]
  ...
  (let [[slider get-slider] (react/useState 0)
    ...
    [:div {:style (merge
     ...
     [:div {:style (m7/css
      ...
      #_[m7/m '[= [:m 1000 J]  [:m 10 g 0.45 [J g] C [:b [- [:k T 2] [:k T 1]]]] ]]
     ...
     [:div {:style (m7/css
      ...
      ;; small
           ...
           (map
            (fn [i]
              [:path {:d (path [(+ ax-dy (* i 2)) -400 :l 0 1200 ])
                      ...
                      :stroke (hsl [(if (= i small-mark) 2 4) 70 70 0.5])
                      :stroke-width 0.4
                      :fill :none}
               ])
            (range 0 11))]
          ...
          [:path {:d (path [0 0 :l 20 0])
                  :id :grf22
                  :stroke (hsl [1.2 30 40 0.8])
                  :stroke-dasharray (m7/space [1.9 0.1])
                  :stroke-width 0.2
                  :fill :none}
  ...
  (let [[slider get-slider] (react/useState 0)
    ...
    [:div {:style (merge
                 ...
                 (let [x (* x1 (/ 40 smb))]
                   [:g
                    ...
                    [:path {:d (m7/path `[ ~x ~y1 :l ~@(map #(* (/ 40 smb) %) dx)])
                            :fill (hsl  [(if (not y) 1.5 2.3) 90 60 0.8])
                            :stroke :#aaa
                            :stroke-width 2
                            }]
                    [:text {:x x
                ...
                (let [x (* x1 40)]
                  [:g
                   ...
                   [:path {:d (m7/path `[ ~x ~y1 :l ~@(map #(* 40 %) dx)])
                           :fill (hsl  [(if (not y) 0.1 0.8) 90 60 0.5])
                           :stroke :#aaa
                           :stroke-width 2
                           }]
                   [:text {:x x
     ...
     [:div {:style
      ...
      #_(if true
        [:div {:style {:background-color (hsl [1 50 50 0.7])}}
         [m7/mx `[= [:p [:b [+ ~ax ~bx]]  2]
  ...
  (let [[slider get-slider] (react/useState 0)
    ...
    [:div {:style (merge
     ...
     #_[:div {:style
      ...
      #_[:div {:style {:background-color (hsl [0.6 95 70 1])}} "if we substitute " [m7/mx `[:a x [+ x [:k d x]]]]]
  ...
  (let [[slider get-slider] (react/useState 0)
    ...
    [:div {:style (merge
     ...
     [:div {:style (m7/css
      ...
      #_[m7/m '[= 45.7395 [+ 45.73 0.0095] ]]

      #_[m7/m '[= 45.7395 [+ 45.73 0.01] ]]
        ...
        (let []
          [:svg {:style {:height "100%"
           ...
           [:path {:d (path [160 0 :a 160 160 0 true false   -160 160])
                   :stroke (hsl [0 70 70 1])
                   :stroke-width 2
                   :transfrom (m7/tranfrom [[:scale 0.3]])
                   :marker-end (m7/url (name :dot))
                   :fill :none}]
  ...
  (let [points [
        ...
        (map (fn [quadro]
               (let [[start-p [x1 y1 :as start] start-n _] quadro
                     ...
                     start-len (* 0.25 (line-len start-p start-n))
        ...
        (let []
          [:svg {:style   {:height "100%"
           ...
           (map (fn [x y]
                ...
                (map (fn [x]
                       (* 20 x)) (range -10  20 0.25))
                (map eqs2 (range -10 20 0.25)))
           ...
           [:g
            ...
            [:circle {:cx (* 8 20)
                 ...
                 [:circle {:on-mouse-over
                           ...
                           :cx (* 0 20)
                           :cy (ve (+ (* 2 -6) (* 20 -1)))
                           :fill (hsl [(if pos 0 0.8) 90 60 0.8] )
                           :r 2}
                  [:animate {:attributeName :r
                             :from 1
                             :to 2
           ...
           [:g {:transform (m7/tranfrom [[:translate [100 20]]])}
            ...
            [:g#flux-co2
             [:path {:d (m7/path [0 0 :l 50 0
                     ...
                     :transform (m7/tranfrom [[:scale [0.5 0.5]]])
                     :fill (m7/url (name :starc2))
                     :stroke (hsl [4 70 70 1])
                     :stroke-width 1}]
       ...
       [:div {:style (m7/css
        ...
        (let []
          [:svg {:style   {:height "100%"
           ...
           [:defs
            [:linearGradient {:id "morningGradient" :gradientUnits "userSpaceOnUse" :x1 "0%" :y1 "0%" :x2 "100%" :y2 "0%"}
             [:animate
              {:attributeName "x1" :values "0%;100%" :dur "10s" :repeatCount "indefinite"}]
             [:stop {:offset "0%" :stop-color "#89CFF0"}]
             [:stop {:offset "30%" :stop-color "#A5D8F3"}]
             [:stop {:offset "70%" :stop-color "#E6F4F1"}]
             [:stop {:offset "100%" :stop-color "#F9F7E8"}]]]
           [:rect {:width "200" :height "200" :fill "url(#morningGradient)"}]
        ...
        (let []
          [:svg {:style {:height "100%"
           ...
           [:linearGradient {:x1 0.5
            ...
            [:stop  {:offset 0.33
                     :stop-color (hsl [2.3 70 70 0.7])}]
            [:stop  {:offset 0.77
                     :stop-color (hsl [3 70 70 0.4])}
             [:animate {:attributeName :offset
                        :from 0
                        :to 1
                        :dur (m7/not-space [3 "s"])
        ...
        (let []
          [:svg {:style {:height "100%"
           ...
           [:g
            ...
            [:text {:fill (hsl [0.5 65 65 1])
                    :stroke (hsl [0.1 70 60 1])
           ...
           #_[:circle {:cx 0
                     ...
                     :stroke-dasharray 0
                     :stroke-width 10
                     :fill (hsl [0 70 70 0.75])
                     :stroke (hsl [3 70 70 0.75])}
        ...
        (let []
          [:svg {:style {:height "100%"
           ...
           [:radialGradient {
            ...
            [:stop  {:offset 0.33
                     :stop-color (hsl [0.2 70 70 0.9])}]
            [:stop  {:offset 0.77
                     :stop-color (hsl [1 70 70 0.8])}
             [:animate {:attributeName :offset
                        :from 0.88
                        :to 1
                        :dur (m7/not-space [3 "s"])
           ...
           [:radialGradient {
            ...
            [:stop  {:offset 0.77
                     :stop-color (hsl [4.3 70 70 0.3])}
             ]

            [:stop  {:offset 0.97
                     :stop-color (hsl [1 90 80 0.2])}
             ]
            [:animate {:attributeName :offset
                       :from 0.33
                       :to 1
...
(comment
  (if (and svg-ref (-> svg-ref .-current))
                                     (do
      ...
      {:ref (if (and (= slider 0)
       ...
       :style (m7/css
               [[3 6 12 23 :center :center 1.5 :rem :column]
                [2 70 90 0.5] [] {:gap "1rem"
                                :z-index 5}])}


      [m7/x  `[= [[- ya yb] [- xa xb]] [[- -5 1] [- 0 [- 2]]] [-6 2]]]
      [m7/x `[= [:p AB 2] [+ [:p [:b -6] 2]  [:p 2 2]] 40]]
...
(defn playgroud []
  (let [f (fn [n] (/ 1 n))
        ...
        [real-slider set-real-slider] (react/useState 50)
    ...
    [:div {:style (merge
     ...
     [:div
      {:ref (if (and (= slider 0)
              ...
              text-ref nil)
       :style (m7/css
               [[3 6 1 23 :center :center 2.5 :rem]
                [2 70 90 0.3] [] {:gap "1rem"
                                :z-index 3}])}
      ...
      (if (> count 0)
        [:div
         ...
         ;; [:div [m7/x `[= [:m f [:b x]] [:m 9 x]]]]


         ;; [:div [m7/x `[= cb [:m f [:b x]] [:m 60 x]]]]


         ;; [:div [m7/x `[= cb [:m f [:b x]] [:m 60 x]]]]


         ;; [:div [m7/x `[= gg [:m f [:b x]] [:m 5.5 x]]]]

         ;; [:div [m7/x `[= gg [:m f [:b 17.3]] [* 5.5 17.3]]]]

         #_[:div [m7/x `[= y [:m f [:b x]] [:m [1 2] x]]]]

         #_[:div [m7/x `[= [:m f [:b 0]] 0]]]

         #_[:div [m7/x `[= [:m f [:b 10]] [* 2 10] 20]]]

         #_[:div [m7/x `[= [:m f [:b 5]] 10]]]


         #_[:div [m7/x `[= [:m g [:b [:m f [:b  x]]]] _]]]
         #_[:div [m7/x `[= [:m g [:b y]] [+ [:p x 2] 3]]]]
         #_[:div [m7/x `[= [:m g [:b [:m f [:b  x]]]] [+ [:p x 2] 3]]]]
         ])]
           ...
           [:g
            ...
            (if (and (= slider 0) (= clear false))
              ...
              [:g
               ...
               [:path#b1 {:d (m7/path [(* -5 20)
                                       (* -0.5 -5 20)
                                       :L (* 5 10 20) (* -0.5 10 5 20)])
                          :stroke-width 0.5
                          :marker-end (m7/url (name :mb2))
                          :marker-start (m7/url (name :mb2))
                          :stroke (hsl [2 30 30 1])
               ...
               #_[:path#b1 {:d (m7/path [(* -5 20) y11 :L (* 5 20) y22])
                          :stroke-width 0.5
                          :marker-end (m7/url (name :mb2))
                          :marker-start (m7/url (name :mb2))
                          :stroke (hsl [2 30 30 1])
               ...
               #_[:path#b2 {:d (m7/path [(* -5 20) y44
                          ...
                          :fill (hsl [1 70 70 1])}]
               #_[:path#b3 {:d (m7/path [(* 1 20) 0 :l (* 16 2) 0])
                          :stroke (hsl [1 20 20 1])
                          :stroke-width 0.5
        ...
        (let []
          [:svg {:style {:height "100%"
           ...
           [:linearGradient {:x1 0.5
                             ...
                             :id (name :lgg1)
                             :gradientTransform
                             (m7/tranfrom [[:rotate 10]])}
            [:stop  {:offset 0
                     :stop-color (hsl [1 70 70 0.9])}]
            [:stop  {:offset 0.33
                     :stop-color (hsl [0.3 70 70 0.7])}
             [:animate {:attributeName :offset
                        :from 0.2
       ...
       [:div {:style (m7/css
        ...
        (let []
          [:svg {:style {:height "100%"
           ...
           [:g
            #_[:path {:d (m7/path [0 0 :l 0 (ve (* 2 30 ))
                                 20 0 0 (* 2 30 )
                                 -20 0])
                    :stroke-width 1
                    :fill (hsl [2 70 70 0.7])}]
            ...
            [:path#tri3 {:d (m7/path [ 20 (ve (* 2 30 )) :l
                                      (* 20 2) 0
                                      0 (ve (* 20 1))
                                      (ve (* 2 20)) (* 20 1)
                         ...
                         :stroke (hsl [0.5 70 70 1])
                         :stroke-width 0.5
                         :fill (hsl [1 70 70 0.7])}]


            [:path#tri5 {:d (m7/path [20 (ve (* 2 30 )) :l
                                      (* 20 2) 0
                                      0 (ve (* 20 1))
                                      (ve (* 2 20)) (* 20 1)

                                      ])
                         :transform (m7/tranfrom [[:scale 1.5]])
                         :stroke (hsl [0.5 70 70 1])
                         :stroke-width 0.5
                         :fill (hsl [1 70 70 0.7])}]


            [:path#tri2 {:d (m7/path [20 (ve (* 2 30 )) :l
                                      (* 20 2) 0
                                      0 (ve (* 20 1))
                                      (ve (* 2 20)) (* 20 1)

                                      ])
                         :transform (m7/tranfrom [[:scale 1.04]])
                         :stroke (hsl [0.5 70 70 1])
                         :stroke-width 0.001
                         :fill :none}]
            ...
            #_[:path {:d (m7/path [ (+ 20 32) 0 :l 0 (ve (* 2 37 ))
                                 20 0 0 (* 2 37 )
                                 -20 0])
                    :stroke-width 0.5
                      :fill (hsl [2 70 70 1])}]
...
"]
        ...
        #_(let [d 'ε]
          ...
          #_[m7/m [1 1]]
          #_[m7/m '[< [:p 0.00000000000000001 2] 0.00000000000000001 ]]
       ...
       [:div {:style (m7/css
        ...
        #_[:span {:style {:background-color (hsl [1 70 70 0.5])}}"9th October"]
        ...
        (reduce
         (fn [acc e]
           (if (some #(= e %)  ["was" "trotting" "came"
             ...
             (conj acc
                   [:span " "
                    [:span {:style {:background-color (hsl [0.5 95 70 1])
                                    :color (hsl [1 40 40 1])
                                    :font-weight 600}}
                     (str  e)]])
       ...
       [:div {:style (m7/css
        ...
        (let []
          [:svg {:style {:height "100%"
           ...
           [:g
            [:g#logo {:transform (m7/tranfrom [[:scale [0.3 0.3]]])}
             (map
             (fn [x]
               [:g
               ...
               [:g
                ...
                [:path
                 {:d
                  ...
                  :stroke (hsl [4 70 70 0.1])
                  :stroke-width 10
                  :fill  (hsl [0 70 70 0.1])
                  }
                 ]
    ...
    (let [zoom 4
      ...
      [:div {:style (merge
       ...
       [:div {:style (m7/css
        ...
        [m7/x `[= [:sq [2 [:p 2 2]]] x]]
        [m7/x `[= [[:sq 2] 2] x [:m sin 45 ~deg] [:m cos 45 ~deg]]]

        [m7/x `[= [[:sq 2] 2] x [:m sin [pi 4]] [:m cos [pi 4]]]]
        ]
       ...
       [:div {:style (m7/css
        ...
        [m7/m '[= [- 10.96 7.67] TB]]
        [m7/mx `[= [:p AC 2] [:p ~r 2] 144]]

        [m7/mx `[= [:p ~adj 2] 120.1216  [:p BC 2]]]



        [m7/mx `[= [:p AB 2] [:p ~op 2] 156.25]]

        [m7/mx `[= [:p AC 2] [+ [:p AB 2] [:p BC 2]]]]

        [m7/mx `[= 23.8784 [:p AB 2]]]
        ...
        [m7/mx `[= [- 225 112 0.5721] [:p BC 2]]]

        [m7/mx `[= [- [+ 112 1] 0.5721] [:p BC 2]]]


        [m7/mx `[= 112.4279  [:p BC 2]]]
        [m7/mx `[= [- 225 112.5721] [- [- 225 112]  0.5721]]]

        [m7/mx '[= BC [:sq 112.4279] 10.6 ]]

        [m7/m '[:m 2 2 5 . 0 0 0 0]]
        [m7/m '[:m  1 1 2 . 0 0 0 0]]



        [m7/mx `[= BC ~adj]]

        [m7/mx `[= ~(symbol (str "tan" (name tt)))
                 [[:k opp 1] [:k adj 1]]]]
        ...
        [m7/mx `[= [:m r ~(symbol (str "tan" (name tt)))] [:k opp 1]]]
        ...
        [m7/mx `[= BC [* ~r [:m cos [:p ~(fix (/ 180 tn) 1)
                                  ~deg]]]
                 ~adj]]
        ...
        [m7/mx `[= [:m [:k B 1] C]  [* ~r [:m cos [:p ~(fix (/ 180 tn2) 1)
                                  ~deg]]]
                 ~adj2]]
    ...
    (let [zoom 4
      ...
      [:div {:style (merge
       ...
       [:div {:style
              (m7/css
               [[2 8 2 8
                 :center :center  2.1 :rem :column]
                [2.5 70 (+ 50 (* 5 5))  0.7] []
                {:gap ".1rem"
                 :z-index 10}])}


        [:div "14 btc is 309639.40$, how much btc you will get with 500$"]
        ...
        [m7/x `[= u [:m [14 309639.40] [btc $]]]]


        [m7/x `[= b [:m [14 309639.40] d ] ]]


        [m7/x `[= b [* [14 309639.40] 500 ] ]]

        [m7/x `[= b [7000 309639.40] ]]
        ...
        [:div {:style {:background-color (hsl [0 70 70 0.31])}} ""]

        ]

       [:div {:style (m7/css
                        [[2 10 10 20 :center :center 4.5 :rem :column]
        ...
        #_[m7/x `[= 14 [* u 309639.40]]]


        #_[m7/x `[= b [* u 500]]]

        [m7/x `[= [b 14]  [[* u 500] [* u 309639.40]]]]

        #_[m7/x `[= [ [:m 14 b] 14]  [[* 14 500] 309639.40]]]

        #_[m7/x `[= b  [[* 14 500] 309639.40]]]
        ...
        (let []
          [:svg {:style {:height "100%"
           ...
           [:g {:transform (m7/tranfrom [[:skewX 0]])}
            ...
            #_[:g {:transform (m7/tranfrom [[:translate [100 -10]]
                                          [:scale 2]])}
    ...
    (let [zoom 4
      ...
      [:div {:style (merge
       ...
       [:div {:style
        ...
        #_[:div "Unit Price per chocolate/ˈtʃɒk(ə)lət/"]


        #_[m7/mx '[= unit [:m 20 [cent chocolate]]]]



        [m7/mx `[= pirce [* unit  quantiy]]]
        ...
        #_[m7/m '[= [:m [* 0.6 [:p 10 -3]] [:p m 3]] [:k V crown]]]
        ...
        #_[m7/m '[=  [:k ρ a] [[:m [:m 10 kg] ρ] [:m 0.6 kg]]  ]]

        #_[m7/m '[=  [:k ρ g]  [m V] [ [:m [:m 10 Kg]  [:p 10 3] [kg [:m m 3]] ] [:m 0.6 Kg]]]]
          ...
          [:svg {:style {:height "100%"
           ...
           [:g
            ...
            (let [x -60
                  y 95]
              [:g
               #_[:g  {:transform (m7/tranfrom [[:scale [1 1]]])}
               ...
               #_[:path {:d (m7/path `[70 ~y :l ~@(map #(* 50 %) dx)])
            ...
            [:g
             ...
             (let [f1 [0 120]
               ...
               [:g
                ...
                #_[:path {:d (m7/path `[0 0 :l ~@(let [[x y] f1]
                                                 [x (ve y)])])
                        :id :paswbar2
                        :stroke-width 2
                        :marker-end (m7/url (name :dot3))
                ...
                [:animateTransform
                 ...
                 (let [[x1 y1] f1
                   ...
                   [:path {:d (m7/path [0 0 :l (+ x1 x2) (+ y1 y2) ])
                        ...
                        :stroke-width 0
                        :marker-end (m7/url (name :dot3))
                        :stroke (hsl [4.5 70 80 0.9])
                        :fill (hsl [4.5 70 80 0.9])}
    ...
    (let [zoom 4
      ...
      [:div {:style (merge
       ...
       [:div {:style
        ...
        [m7/m '[= P [:m ρ  h g]]]
        ;; [m7/m '[= [:k P o] [:m   12 km  ρ g]]]
        ;; [m7/m '[= [:k P po] [:m   12 km ρ g]]]

        ;; [m7/m '[= [:k P p] [:m   24 m [:b [* 12 ρ]] g]]]


        ;; [m7/m '[:m 1  g [:p cm -3]]]
        ;; [m7/m '[= [:k P bath] [:m ρ  12 km g]]]
        ...
        ;; [m7/m '[= [:m ρ  [:k V crown]] [- m [:k m r]]]]

        ;; [m7/m '[= [:m [- 94] N] [+ [:m [- 100] N] [:k W d]]]]



        ;; [m7/m '[= [:m 6 N] [:k W d] [:m 10 ρ [:k V crown] N]]]


        ;; [m7/m '[= [:m [* 0.6 [:p 10 -3]] [:p m 3]] [:k V crown]]]



        ;; [m7/m '[=  [:k ρ crown] [[:m 10 Kg] [:k V crown]]]]

        ;; #_(/ (js/Math.pow 10 5) 6)
        ;; [m7/m '[=  [:k ρ crown] [[:m [:p 10 5]  Kg] [:m 6 [:p m 3]]]]]
            ...
            (let [x -60
              ...
              [:g
               [:g  {:transform (m7/tranfrom [[:scale [1 1]]])}
                ...
                [:path {:d (m7/path `[-60 95 :l ~@(map #(* 120 %) [0 -1 0 1 1 0 0 -1 ])])
                        :stroke-dasharray (m7/space [10 2])
             ...
             (let [f1 [0 120]
               ...
               [:g
                ...
                #_[ship [[:translate [-60 10]]
                         [:scale [0.01 -0.01]]]]
                ;; cube
                [:path {:d (m7/path `[-25 50 :l ~@(map #(* 50 %) dx)])
                          :stroke-dasharray (m7/space [100 50])
                          :stroke (hsl [1 70 70 1])
                          :stroke-width 2
                          :fill (hsl [1 70 70 1])}]

                (water [[:translate  [-60 0]]
                          [:scale [-1 1]]])
...
;;         [m7/m '[= KE  [:m [1 2] 0.2 [:p [:b [- [:m 30 [m s]] [* [:m 10 [m [:p s 2]]]
;;                                                              [:m 2 s]]] ] 2]]]]
            ...
            (
             (fn [n n1]
               [:g {:transform (m7/tranfrom [[:translate [0 0]]])}
                (map (fn [i]
                       [:path {:stroke-width 2
                               :fill (hsl [(if (< i n1)  3.5 1) 70 70 0.6])
                               :stroke (hsl [7 70 70 1])
                               :d (m7/path
                                   `[~(* i 40) 0 :l ~@(map #(* 20 %1 %2)
                                                           dx
            ...
            (
             (fn [n n1]
               [:g {:transform (m7/tranfrom [[:translate [0 0]]])}
                (map (fn [i]
                       [:path {:stroke-width 2
                               :fill (hsl [(if (< i n1)  0.5 6) 70 70 0.6])
                               :stroke (hsl [7 70 70 1])
                               :d (m7/path
            ...
            (
             (fn [n n1]
               [:g {:transform (m7/tranfrom [[:translate [0 0]]])}
                (map (fn [i]
                       [:path {:stroke-width 2
                               :fill (hsl [(if (< i n1)  3.5 1) 70 70 0.8])
                               :stroke (hsl [7 70 70 1])
                               :d (m7/path
                                   `[~(* i 40) 0 :l ~@(map #(* 20 %1 %2)
                                                           dx
    ...
    (let [zoom 4
      ...
      [:div {:style (merge
       ...
       [:div {:style
        ...
        #_[m7/x `[= ~(* 9 13) [:m 9 [:b [+ 12 1]]] [+ [* 9 12] [* 9 1]] ]]


        #_[m7/x `[= [117 9]
                [+ [99 9]   [9 9] [1 9]  [1 9]  [7 9]] ]]
        ...
        #_[m7/x `[= [* 11 19] [:m 11 [:b [+ 10 9]]] [+ 110 99]]]
        ...
        #_[m7/x `[= ~(* 11 19) [:m 11 [:b [- 20 1]]] [- [* 11 20] [* 11 1]]]]

        #_16387
        #_[m7/x `[= [- 1638 [* 7 2]] [- 1638 14] 1624 ]]
        #_[m7/x `[= [- 1638 [* 7 2]] [- 1638 14] 1624 ]]
        #_[m7/x `[= [- 162 8] 154]]

        #_[m7/x `[= 154 [+ [* 7 2 10] [* 7 2 1]]]]
        ...
        #_[m7/x `[= y [:m f [:b x]] [+ x 7]]]


        #_[m7/x `[= y [:m g [:b x]] [:p x 2]]]
        ;; [m7/x `[=
        ;;         [+ [:m 5 x ] [:m 3 x]]
        ;;         [:m x [:b [+ 5 3 ]]]
        ;;         [:m 8 x]]]
  ...
  (let [[slider get-slider] (react/useState 0)
    ...
    (let [zoom 4
      ...
      [:div {:style (merge
                     ...
                     {:background-size :cover
                      :background-position :center
                      :background-color (hsl [2 100 100 1])
                      ;; :background-image (str "url(" (.getItem js/localStorage "file-input-img" )   ")")
                      :gap ".1rem"})}
       ...
       [:div {:style (m7/css
        ...
        (let []
          [:svg {:style {:height "100%"
           ...
           [:path {:d "M10 10 L190 10 L190 190 L10 190 Z" :fill "url(#myGradient)"}]
